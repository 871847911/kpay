<template>
  <div class="panel pay-panel">
    ...
    <div class="content">
      ...
      <div class="box">
        <table v-if="columnList && columnList.length !== 0">
          <tr>
            <th style="width: 15%">默认费率</th>
            <th style="width: 25%">收取服务费率</th>
            <th style="width: 25%">关联生产者人数</th>
            <th style="width: 35%">操作</th>
          </tr>
          <tr v-for="item of columnList" :key="item.ruleId">
            <td>
            ...
            <td>
              <el-row class="action-row">
                ...
                <div class="delete" @click="item.deleteRow = !item.deleteRow" v-if="item.ruleDefault !== 1">
                ...
                <div class="delete" style="width:0.16rem;" v-else></div>
              </el-row>
            </td>
          </tr>
        </table>
        ...
        <div class="more-setting">
          <p>结算时间
            <el-popover
              ...
              width="200"
              trigger="hover"
              content="按照专栏的更新进度，和生产者进行结算。例专栏价格为100元，当前进度为1/10，以费率1%计算，结算给生产者的费用为9.9元。">
              <p slot="reference">
          ...
          <div class="set-radio">
            <el-radio v-model="radioSettle" label="2">手动结算</el-radio>
            <el-radio v-model="radioSettle" label="1">自动结算</el-radio>
          </div>
          <div class="set-remind">
            <p>注：手动结算：满足结算规则后，可结算金额会进行累计，并需要触发【结算】操作，才会最终结算</p>
            <p>自动结算：满足结算规则后，系统自动结算</p>
          </div>
        </div>

        <div class="btn-box">
          <el-button class="save" @click="updateStoreConfig">保存设置</el-button>
        </div>
      </div>
      ...
      <!-- 新增费率 -->
      <el-dialog title="新增费率" :visible.sync="dialogAddRateVis" width="800px">
      ...
      <el-dialog title="关联生产者" :visible.sync="dialogConectVis" width="840px">
        <div class="producer-dialog">
          ...
          <p class="remind">付费专栏费率 {{showRate}}，结算时间 {{showTime}}，已选择<span>{{conntentLen}}</span>个生产者</p>
      ...
      <el-dialog title="查看生产者" :visible.sync="dialogLookVis" width="840px">
        <div class="producer-dialog">
          ...
          <p class="remind">付费专栏费率 {{showRate}}，共有<span>{{alreadyConnectPerson.length}}</span>个生产者</p>
...
export default {
  ...
  data () {
    ...
    return {
      ...
      rowItem: {}, // 点击单条临时存储变量
      radioSettle: '2' // 手动或者自动结算
    }
  },

  props: {
    columnList: Array,
    autoSettlement: Number
  },
  ...
  created() {
    if (this.autoSettlement) {
      this.radioSettle = this.autoSettlement.toString()
    }
  },

  watch: {
    autoSettlement(newVal, oldVal) {
      if (newVal) {
        this.radioSettle = newVal.toString()
      }
    }
  },
  ...
  methods: {
      ...
      this.allFormRulesPass(formName).then(valid => {
        if (valid) {
          ...
          api.updateServiceRate({
            ...
            if (res.success) {
              this.msgNotify('更改成功')
              this.$emit('columnChangeItem', ruleId, newRate)
            } else {
    ...
    delColumnContinue() {
      ...
      api.deletePayStoreRule({
        ...
        if (res.success) {
          this.msgNotify('删除成功')
          this.$emit('columnDelItem', ruleId)
        } else {
    ...
    // 修改专栏结算方式
    updateStoreConfig() {
      api.updateStoreConfig({
        autoSettlement: this.radioSettle
      }).then(res => {
        if (res.success) {
