    ...
    <div class='steptwo'>
        ...
        <div class="serch_box">
            <div class="serch_line">
                        <el-input class="width240" placeholder="请输入课程编号" maxlength="30" v-model="serchForm.classNumber" ></el-input>
                        <el-input class="width240" placeholder="请输入课程名称" maxlength="20" v-model="serchForm.className" ></el-input>
                        <el-select class="width100" v-model="serchForm.isShareValue" placeholder="是否添加">
                            <el-option v-for="item in serchForm.isShareOptons" :key="item.value" :label="item.label" :value="item.value"></el-option>
                        </el-select>
                        <el-input class="width100" placeholder="起始章节" v-model="serchForm.startNumber" @change="startPage"></el-input>
                        ---
                        <el-input class="width100" placeholder="结束章节" v-model="serchForm.endNumber" @change="lastPage"></el-input>
        ...
        <div class="serch_btn">
            ...
            <el-button class="main_btn_bg" type="primary" size="small" :disabled="serchForm.isShareValue == 1" @click="addMultipleSelection">加入分享</el-button>
            <el-button type="info" size="small" @click="deleteClassListArr" :disabled="serchForm.isShareValue == 2">取消分享</el-button>
        </div>
        <div class="total_box">
        ...
        <div class="table_box">
            <el-table ref="multipleTable" :data="tableData3" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange" :default-sort = "{prop: 'date', order: 'descending'}" @sort-change="sortChange">
                ...
                <el-table-column align="center" prop="courseName" label="课程名称" width="250px">
                    <template slot-scope="scope">
                        <div class="table_class_name_left">
                            <img :src="$store.state.imageHead + scope.row.courseUrlFirst " width="50px" height="50px" alt="">
                        </div>
                ...
                <el-table-column align="center" prop="chapterNum" label="章节" ></el-table-column>
                <el-table-column align="center" prop="chapterNum" label="交易" sortable="custom"></el-table-column>
    ...
    methods: {
        ...
        getList(){
            let params = {
                ...
                chapterNumStart : this.serchForm.startNumber,
                chapterNumStop : this.serchForm.endNumber
            }
            ...
            api.getStepTwoClassList(params).then(res => {
                ...
                if(this.serchForm.isShareValue == 1){
                    ...
                    if(res.list.length > 0 ){
                        ...
                        //         if(res.list[i].id = this.tableData3[j].id){
                        ...
                        res.list.forEach(tableVal => {
                            this.tableData3.forEach((listVal,index) => {
                                if(tableVal.id == listVal.id){
                                    // this.$refs.multipleTable.toggleRowSelection(tableVal)
        ...
        startPage(val){
            ...
            if(!exp.test(val)){
                this.serchForm.startNumber = null;
                this.$message.error("请正确输入章节")
            }
        },
        lastPage(val){
            let exp = new RegExp('[0-9]');
            if(exp.test(val)){
                ...
                if(val < this.serchForm.startNumber){
                     this.serchForm.endNumber = null;
                     this.$message.error("结束章节必须大于开始章节")
                }
            }else{
                this.serchForm.endNumber = null;
                this.$message.error("请正确输入章节")
