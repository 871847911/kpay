                                ...
                                <el-form-item prop="price">
                                  <div class="left">
                                    <p class="p_same">专栏价格</p>
                                     <el-input  placeholder="请输入价格" v-model="formData.price" :disabled="aiticleState" auto-complete="off"></el-input>
...
<script>
import HTitle from "../../common/htitle.vue";
...
export default {
  ...
  data() {
    ...
    return {
      rules: {
        columnName: [{ validator: spacialWord, trigger: "blur" }],
        columnUrl: [{ required: true, message: "请上传封面", trigger: "blur" }],
        price: [{ validator: validatePercent, trigger: "blur" }],
        articleNumber: [{ validator: validatePercent, trigger: "blur" }],
        frequency: [{ required: true, message: "请选择日期", trigger: "blur" }]
      },
      audioName: "",
      activeName: "first",
  ...
  methods: {
    isinit(){
      ...
      this.formData.columnType = this.roleId === 1 ? 2 : 1;
    },
    ...
    initTemplate() {
      ...
      api
        .getIntroductionInfo({ columnId: this.$route.query.editorId })
        .then(res => {
          if (res.success) {
