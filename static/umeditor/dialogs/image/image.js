(function() {
	...
	function initUmeditorQiniuWebUploader(me) {
		// 不用重复初始化
		if (initUmeditorQiniuWebUploader.inited)
			return;
		initUmeditorQiniuWebUploader.inited = true;
		// var qiniuTokenUrl=me.editor.getOpt('qiniuTokenUrl');
		// var qiniuTokenKey=me.editor.getOpt('qiniuTokenKey');
		var qiniuImgPrefix = me.editor.getOpt('qiniuImgPrefix');
		var qiniuShowedUrl = me.editor.getOpt('qiniuShowedUrl');
		var qiniuUploadUrl = me.editor.getOpt('qiniuUploadUrl');
		var data = me.editor.getOpt('qiniuToken')
		// $.post(qiniuTokenUrl,function(data){
		UM.UmeUploader = WebUploader.create({
			auto: true,
			server: qiniuUploadUrl,
			formData: {
				"token": data,
				'accept': 'text/plain'
			},
			...
			accept: {
				...
				mimeTypes: 'image/gif,image/jpg,image/jpeg,image/bmp,image/png,image/jpg'
			}
		});

		UM.UmeUploader.on('uploadBeforeSend', function(block, data, headers) {
			var newTime =
				fileTime.getFullYear() +
				Appendzero(fileTime.getMonth() + 1) +
				Appendzero(fileTime.getDate());
			data.key = "temp/" + data.type + "/" + newTime + "/" + fileTime.getTime() + data.id;
		});

		UM.UmeUploader.on('uploadSuccess', function(file, ret) {
			Base.callback(me.editor, me.dialog, (qiniuShowedUrl || "") + ret.key, 'SUCCESS');
			UM.UmeUploader.removeFile(file);
		});

		UM.UmeUploader.on("error", function (type) {
			 // UM.UmeUploader.reset();
		    ...
		    // }
		   // me.toggleMask();
		   // Base.callback(me.editor, me.dialog, '','Error!');
		});
	}
	...
	var Upload = {
		...
		init: function(editor, $w) {
			...
			initUmeditorQiniuWebUploader(me);
			...
			$(me.dialog).delegate(".edui-image-file", "change", function(e) {
				...
				$.each(fileList, function(i, f) {
					if (/^image/.test(f.type)) {
						//创建图片的base64
						UM.UmeUploader.addFiles(f);
						UM.UmeUploader.upload();
						hasImg = true;
					}
				});
				// if (hasImg) {
